...
pub_world_pose_dyn = rospy.Publisher(
    '/target_pose_meas_cov_stamped_panda_link0', PoseWithCovarianceStamped, queue_size=1)
pub_world_pose_stat = rospy.Publisher(
    'object_pose_w', PoseWithCovarianceStamped, queue_size=1)
...
def find_pose(r, c, z_c):
    ...
    center_point.pose.pose.position = Point(x_w, y_w, z_w)
    center_point.pose.pose.orientation = Quaternion(0, 0, 1, 0)

    pub_world_pose_stat.publish(center_point)
    return pose
...
# The camera will start the detection and make it last for the duration defined here.
# When the detection is finished the coordinates will be sent to the mission planner to be used.
...
def detect_centers():
    ...
    # while len(boxes_ids) <= 0:
    while duration < DETECTION_DURATION:
        ...
        # Crop the region of interest (ROI) ---- TABLE
        cropping_y = 200
        cropping_x = 350
        ...
        for box_id in boxes_ids:
            ...
            detected_objects.append([id, cx_w, cy_w, cz_w])


sys.stdout.write('\n Ending object detection\n')

# -------------- ONLY ON DYNAMIC TASK------------
# As soon as the objeacts leaves the detecting area we stop publishing it
# left_border = cx - w/2
# if left_border > cropping_x:
#     pub_world_pose.publish(center_point)
...
def detection(msg):
    ...
    if message == 100:
        try:
            # Let's call the function to detect the centers of the objects for a predefined duration, in this case 3 seconds
            detect_centers()

            sys.stdout.write('\nLast detected objects to pick up:\n')

            # Iterating through all the identified objects to publish them
            for object in detected_objects:
                id, cx_w, cy_w, cz_w = object
                sys.stdout.write('\nX: ' + str(cx_w) + ' Y: ' +
                                 str(cy_w) + ' Z: ' + str(cz_w) + ' With ID: ' + str(id) + '\n')
            ...
            if len(detected_objects) == 0:
                pub_status.publish("No objects detected!")
            else:
                pub_status.publish("Objects detected!")
...
if __name__ == '__main__':
    ...
    # This will receive String messages from the main mission and if it contains some
    # specifi text then the detection will start.
    rospy.Subscriber('start_detection', Int32, detection)
    rospy.Subscriber('check', String, check)
