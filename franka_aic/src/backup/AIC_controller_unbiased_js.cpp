...
namespace franka_aic
{
  ...
  bool AICController::init(hardware_interface::RobotHW *robot_hw, ros::NodeHandle &node_handle)
  {
    ...
    ROS_WARN("UNBIASED AICController - Joint Space: Ready to start the magic?! :OOOO");
  ...
  void AICController::starting(const ros::Time & /*time*/)
  {
    ...
    mu_d << 0.0, 0.0, 0.01, -1.57, 0.0, 1.57, -1.57; // Center
    mu_dLeft << 1.57, 0.01, 0.01, -1.57, 0.0, 1.57, -1.57;
    mu_dPick << 0.0, 0.15, 0.01, -2.24, 0.01, 2.39, 0.37;
    mu_dCenter << 0.0, 0.0, 0.0, -1.57, 0.0, 1.57, 0.0;
    mu_dRight << -1.57, 0.01, 0.0, -1.57, 0.0, 1.57, -1.57;
    ...
    var_mu = 20.0;      // gru_update
    var_muprime = 30.0; // gru_update
    var_q = 1;          // gru_seen
    var_qdot = 1;       // gru_seen
    ...
    {
      SigmaP_yq0(i, i) = 1 / var_q;           // gru_seen
      SigmaP_yq1(i, i) = 1 / var_qdot;        // gru_seen
  ...
  void AICController::update(const ros::Time &time, const ros::Duration &period)
  {
    ...
    if (time_passed >= 5.0 && time_passed < 15.0)
    {
      mu_d = mu_dLeft;
    }
    if (time_passed >= 15.0 && time_passed < 25.0)
    {
      mu_d = mu_dCenter;
    }
    if (time_passed >= 25.0 && time_passed < 35.0)
    {
      mu_d = mu_dPick;
    }
    if (time_passed >= 35.0 && time_passed < 45.0)
    {
      mu_d = mu_dCenter;
    }
    if (time_passed >= 45.0 && time_passed < 55.0)
    {
      mu_d = mu_dRight;
    }
