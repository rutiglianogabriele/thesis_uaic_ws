...
namespace franka_aic
{
  ...
  void AICController::starting(const ros::Time & /*time*/)
  {
    ...
    mu_d << 0.0, 0.0, 0.0, -1.57, 0.0, 1.57, -1.57; // Center
    mu_dLeft << 1.57, 0.0, 0.0, -1.57, 0.0, 1.57, -1.57;
    mu_dPick << 0.0, 0.15, 0.00, -2.24, 0.0, 2.39, 0.37;
    mu_dCenter << 0.0, 0.0, 0.0, -1.57, 0.0, 1.57, 0.0;
    mu_dRight << -1.57, 0.0, 0.0, -1.57, 0.0, 1.57, -1.57;
  ...
  void AICController::update(const ros::Time &time, const ros::Duration &period)
  {
    ...
    // Sinusoidal tracking
    // if (time_passed >= 5.0 && time_passed < 25.0)
    // {
    //   // Calculate the sinusoidal value for the first joint
    //   double firstJointValue = amplitude * sin(2 * M_PI * frequency * time_passed);
    //   mu_d << firstJointValue, 0.0, 0.0, -1.57, 0.0, 1.57, -1.57;
    // }
