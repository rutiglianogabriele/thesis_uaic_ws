...
namespace franka_aic
{
  ...
  private:
    ...
    Eigen::Matrix<double, 7, 1> mu, mu_p, mu_pp, mu_dot, mu_dot_p, mu_dot_pp, jointPos, _jointPos, jointVel, mu_past, mu_p_past;
    ...
    // To check if needed.
    Eigen::Matrix<double, 7, 1> mu_d_dot, mu_d_prev;

    // Via-points
    Eigen::Matrix<double, 7, 1> mu_dLeft, mu_dPick, mu_dCenter, mu_dPlace, mu_dRight;
    double amplitude, frequency, duration;
    ...
    sensor_msgs::JointState _limited_j_cmds, _j_cmds, _j_f_cmds, _mu, _mu_dot, _mu_d, _u;
    ros::Publisher pub_limited_j_cmds, pub_j_cmds, pub_j_f_cmds, pub_mu, pub_mu_dot, pub_mu_d, pub_u;
    ...
    // Added due to Analytical IK
    std::array<double, 7> _resultIK;

    // Added due to Filter
    Eigen::Matrix<double, 7, 1> xPrev, yPrev, yn, xn;
